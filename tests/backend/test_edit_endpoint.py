...
def test_undo_redo_restores_transcript(tmp_path: Path) -> None:
    projects_root = tmp_path / "projects"
    with run_server(env={"TEXTAUDIO_PROJECTS_DIR": str(projects_root)}) as base_url:
        status, body = _request_json(
            "POST", f"{base_url}/api/projects", {"metadata": {}}
        ...
        assert status == 200
        assert Transcript.model_validate(updated).to_text() == "Hello, there"

        status, undone = _request_json(
            "POST", f"{base_url}/api/projects/{project_id}/undo"
        )
        assert status == 200
        assert Transcript.model_validate(undone).to_text() == "Hello, world"

        status, redone = _request_json(
            "POST", f"{base_url}/api/projects/{project_id}/redo"
        )
        assert status == 200
        assert Transcript.model_validate(redone).to_text() == "Hello, there"
