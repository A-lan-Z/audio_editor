...
def test_invalid_project_id_returns_422() -> None:
    with run_server() as base_url:
        status, _ = _request_json("GET", f"{base_url}/api/projects/not-a-uuid")
        assert status == 422


def test_invalid_create_project_body_returns_422() -> None:
    with run_server() as base_url:
        status, _ = _request_json(
            "POST", f"{base_url}/api/projects", {"metadata": "nope"}
        )
        assert status == 422
...
def test_request_too_large_returns_413() -> None:
    with run_server(env={"TEXTAUDIO_MAX_BODY_BYTES": "1"}) as base_url:
        ...
        assert status == 413
        assert "Request too large" in body["detail"]
